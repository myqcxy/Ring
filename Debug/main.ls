 ...
 663                     ; 66 	m_tWarnGap=m_Param.tWarnGap;
 665  0074 bed1          	ldw	x,_m_Param+6
 666  0076 bfc3          	ldw	_m_tWarnGap,x
 667                     ; 67 	m_tDataGap=m_Param.tDataGap;
 ...
 881                     ; 143 				Clear_Usart_Rx();	
 883  0150 cd0352        	call	_Clear_Usart_Rx
 886  0153               L363:
 887                     ; 149 			bRx=0;	//restart recv
 889  0153 3f35          	clr	_bRx
 890  0155               L753:
 891                     ; 152 		if(!m_tLLasting && !m_tSLasting)
...
1011                     ; 178 	for(i=0;i<nTimes;i++)
1013  019d 5f            	clrw	x
1014  019e 1f01          	ldw	(OFST-3,sp),x
1016  01a0 2017          	jra	L724
1017  01a2               L324:
1018                     ; 180 		nBase=648;//debug 154£¬release 648;
...
1047  01c1 81            	ret
1081                     ; 187 void Delay_5us(void)	//2M--not precise
1081                     ; 188 {
1082                     	switch	.text
1083  01c2               _Delay_5us:
1085  01c2 89            	pushw	x
1086       00000002      OFST:	set	2
1089                     ; 191 	nBase=32;	//debug 1, release 4, freq 2M;
...
1238                     ; 217 }
1241  0231 81            	ret
1291                     ; 220 void Init_Mem(void)
1291                     ; 221 {
1292                     	switch	.text
1293  0232               _Init_Mem:
1295  0232 88            	push	a
1296       00000001      OFST:	set	1
1299                     ; 224 	m_intr1=0;	//based on TIM2, restricting uart tx & rx
1301  0233 5f            	clrw	x
...
1441                     ; 255 		CLK_PeripheralClockConfig(CLK_Peripheral_TIM2, ENABLE);
1443  0281 ae0001        	ldw	x,#1
1444  0284 a601          	ld	a,#1
1445  0286 95            	ld	xh,a
1446  0287 cd0000        	call	_CLK_PeripheralClockConfig
1448                     ; 256 	  TIM2_TimeBaseInit(TIM2_Prescaler_1, TIM2_CounterMode_Up, 19999);	
...
1490                     ; 264 }
1493  02b3 81            	ret
1532                     ; 267 void Set_Usart(uint8_t b)
1532                     ; 268 {
1533                     	switch	.text
...
1621                     ; 286 }
1624  0318 81            	ret
1670                     ; 289 void Usart_Send(uint8_t Len)
1670                     ; 290 {
1671                     	switch	.text
...
2151                     ; 369 uint8_t RecvData(uint16_t uWait)	//ret: 0--no data, 255--err data, other--valid data
2151                     ; 370 {
2152                     	switch	.text
2153  03e6               _RecvData:
...
2160                     ; 371 	uint8_t b=0;
2162  03e8 0f02          	clr	(OFST+0,sp)
2163                     ; 372 	uint8_t Cmd=0;
2165                     ; 375 	if(!bRxFirst)
...
2191  0404               L5201:
2192                     ; 384 			if(iRxIndex>=MAX_RX_LEN)
...
2201                     ; 387 				if(CheckSumXor(Usart_RxBuff,MAX_RX_LEN))
...
2235  0428 81            	ret
2307                     ; 424 uint8_t GetCheckSumXor(uint8_t* pu8, uint8_t Len)
2307                     ; 425 {
2308                     	switch	.text
...
2374  0464 81            	ret
2478                     ; 444 bool CheckSumXor(uint8_t* pu8,uint8_t Len)
2478                     ; 445 {
2479                     	switch	.text
...
2487                     ; 446 	bool b=FALSE;
2489  0468 0f01          	clr	(OFST-4,sp)
2490                     ; 447 	uint8_t u1=0;
2492                     ; 448 	uint8_t u2=0;
2494                     ; 451 	pDat=pu8;
2496  046a 1f04          	ldw	(OFST-1,sp),x
2497                     ; 452 	u1=GetCheckSumXor(pDat,Len);
...
2539  0498 81            	ret
2581                     ; 463 void EnterAHalt(uint8_t time)
2581                     ; 464 {
2582                     	switch	.text
2583  0499               _EnterAHalt:
2585  0499 88            	push	a
2586       00000000      OFST:	set	0
2589                     ; 465 	CLK_SetTick(0);	
2591  049a 4f            	clr	a
2592  049b cd027e        	call	_CLK_SetTick
2594                     ; 466 	Set_Usart(0);
2596  049e 4f            	clr	a
...
2608                     ; 472 	GPIO_Init(GPIOC, GPIO_Pin_All, GPIO_Mode_Out_OD_HiZ_Slow);    
...
2620                     ; 476 	CLK_PeripheralClockConfig(CLK_Peripheral_AWU, ENABLE);
...
2775  0006 00            	dc.b	0
2834                     ; 508 @far @interrupt void TIM2_Handle_ISR(void)
2834                     ; 509 {
2836                     	switch	.text
...
2923                     ; 539 		if(KeyLasting>10 && KeyLasting<=150)
...
2932                     ; 540 			m_Press=01;			//short press
...
3003  05c3 80            	iret
3182                     ; 571 bool I2C_CheckEvent_T(I2C_Event_TypeDef I2C_Event,uint8_t timeout)
3182                     ; 572 {
3184                     	switch	.text
...
3192                     ; 573 	bool b=FALSE;
3194  05c6 0f01          	clr	(OFST-1,sp)
3195                     ; 576 	for(i=0;i<timeout;i++)
3197  05c8 0f02          	clr	(OFST+0,sp)
3199  05ca 2016          	jra	L1041
3200  05cc               L5731:
3201                     ; 578 		if(I2C_CheckEvent(I2C_Event))
...
3297                     ; 590 bool I2C_SendCmd(uint8_t slvAddr,uint8_t Size)	
3297                     ; 591 {
3298                     	switch	.text
...
3348                     ; 609 		I2C_GenerateSTOP(ENABLE);
3350  0618 a601          	ld	a,#1
3351  061a cd0000        	call	_I2C_GenerateSTOP
3353                     ; 610 		return FALSE;
3355  061d 4f            	clr	a
3357  061e 2029          	jra	L27
3358  0620               L7541:
3359                     ; 617     I2C_SendData(I2c_TxBuff[iTx++]);
...
3398                     ; 627 	return TRUE;
3400  0647 a601          	ld	a,#1
3402  0649               L27:
3404  0649 5b03          	addw	sp,#3
3405  064b 81            	ret
3487                     ; 631 bool I2C_GetData(uint8_t slvAddr, uint8_t *pu8, uint8_t Size)	
3487                     ; 632 {
3488                     	switch	.text
...
3811  072e 84            	pop	a
3812  072f 80            	iret
3835                     ; 720 @far @interrupt void AWU_Handle_ISR(void)
3835                     ; 721 {
3836                     	switch	.text
...
3860  074b 80            	iret
3884                     ; 726 @far @interrupt void EXTI5_Handle_ISR(void)
3884                     ; 727 {
3885                     	switch	.text
