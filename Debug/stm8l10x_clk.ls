  ...
  19  0002 04            	dc.b	4
  20  0003 08            	dc.b	8
  49                     ; 56 void CLK_DeInit(void)
  49                     ; 57 {
  51                     	switch	.text
  52  0000               _CLK_DeInit:
  56                     ; 58     CLK->CKDIVR = CLK_CKDIVR_RESET_VALUE;
  58  0000 350350c0      	mov	20672,#3
  59                     ; 59     CLK->PCKENR = CLK_PCKENR_RESET_VALUE;
  61  0004 725f50c3      	clr	20675
  62                     ; 60     CLK->CCOR  = CLK_CCOR_RESET_VALUE;
  64  0008 725f50c5      	clr	20677
  65                     ; 61 }
  68  000c 81            	ret
 123                     ; 69 void CLK_CCOCmd(FunctionalState NewState)
 123                     ; 70 {
 124                     	switch	.text
 125  000d               _CLK_CCOCmd:
 129                     ; 72     assert_param(IS_FUNCTIONAL_STATE(NewState));
 131                     ; 74     if (NewState != DISABLE)
 133  000d 4d            	tnz	a
 134  000e 2706          	jreq	L74
 135                     ; 77         CLK->CCOR |= CLK_CCOR_CCOEN;
 137  0010 721050c5      	bset	20677,#0
 139  0014 2004          	jra	L15
 140  0016               L74:
 141                     ; 82         CLK->CCOR &= (uint8_t)(~CLK_CCOR_CCOEN);
 143  0016 721150c5      	bres	20677,#0
 144  001a               L15:
 145                     ; 85 }
 148  001a 81            	ret
 253                     ; 95 void CLK_PeripheralClockConfig(CLK_Peripheral_TypeDef CLK_Peripheral, FunctionalState NewState)
 253                     ; 96 {
 254                     	switch	.text
 255  001b               _CLK_PeripheralClockConfig:
 257  001b 89            	pushw	x
 258       00000000      OFST:	set	0
 261                     ; 98     assert_param(IS_FUNCTIONAL_STATE(NewState));
 263                     ; 99     assert_param(IS_CLK_PERIPHERAL(CLK_Peripheral));
 265                     ; 101     if (NewState != DISABLE)
 267  001c 9f            	ld	a,xl
 268  001d 4d            	tnz	a
 269  001e 2709          	jreq	L711
 270                     ; 104         CLK->PCKENR |= ((uint8_t)CLK_Peripheral);
 272  0020 9e            	ld	a,xh
 273  0021 ca50c3        	or	a,20675
 274  0024 c750c3        	ld	20675,a
 276  0027 2009          	jra	L121
 277  0029               L711:
 278                     ; 109         CLK->PCKENR &= (uint8_t)(~(uint8_t)CLK_Peripheral);
 ...
 288  0032 85            	popw	x
 289  0033 81            	ret
 364                     ; 119 void CLK_MasterPrescalerConfig(CLK_MasterPrescaler_TypeDef CLK_MasterPrescaler)
 364                     ; 120 {
 365                     	switch	.text
 366  0034               _CLK_MasterPrescalerConfig:
 368  0034 88            	push	a
 369       00000000      OFST:	set	0
 372                     ; 122     assert_param(IS_CLK_MASTER_PRESCALER(CLK_MasterPrescaler));
 374                     ; 124     CLK->CKDIVR &= (uint8_t)(~CLK_CKDIVR_HSIDIV);
 376  0035 c650c0        	ld	a,20672
 377  0038 a4fc          	and	a,#252
 378  003a c750c0        	ld	20672,a
 379                     ; 125     CLK->CKDIVR = (uint8_t)CLK_MasterPrescaler;
 381  003d 7b01          	ld	a,(OFST+1,sp)
 382  003f c750c0        	ld	20672,a
 383                     ; 126 }
 386  0042 84            	pop	a
 387  0043 81            	ret
 460                     ; 137 void CLK_CCOConfig(CLK_Output_TypeDef CLK_Output)
 460                     ; 138 {
 461                     	switch	.text
 462  0044               _CLK_CCOConfig:
 464  0044 88            	push	a
 465       00000000      OFST:	set	0
 468                     ; 140     assert_param(IS_CLK_OUTPUT(CLK_Output));
 470                     ; 143     CLK->CCOR &= (uint8_t)(~CLK_CCOR_CCOSEL);
 472  0045 c650c5        	ld	a,20677
 473  0048 a4f9          	and	a,#249
 474  004a c750c5        	ld	20677,a
 475                     ; 146     CLK->CCOR |= ((uint8_t)CLK_Output);
 477  004d c650c5        	ld	a,20677
 478  0050 1a01          	or	a,(OFST+1,sp)
 479  0052 c750c5        	ld	20677,a
 480                     ; 149     CLK->CCOR |= CLK_CCOR_CCOEN;
 482  0055 721050c5      	bset	20677,#0
 483                     ; 150 }
 486  0059 84            	pop	a
 487  005a 81            	ret
 540                     ; 159 uint32_t CLK_GetClockFreq(void)
 540                     ; 160 {
 541                     	switch	.text
 542  005b               _CLK_GetClockFreq:
 544  005b 5209          	subw	sp,#9
 545       00000009      OFST:	set	9
 548                     ; 161     uint32_t clockfrequency = 0;
 550                     ; 162     uint8_t tmp = 0, presc = 0;
 554                     ; 164     tmp = (uint8_t)(CLK->CKDIVR & CLK_CKDIVR_HSIDIV);
 556  005d c650c0        	ld	a,20672
 557  0060 a403          	and	a,#3
 558  0062 6b09          	ld	(OFST+0,sp),a
 559                     ; 165     presc = HSIDivFactor[tmp];
 561  0064 7b09          	ld	a,(OFST+0,sp)
 562  0066 5f            	clrw	x
 563  0067 97            	ld	xl,a
 564  0068 d60000        	ld	a,(_HSIDivFactor,x)
 565  006b 6b09          	ld	(OFST+0,sp),a
 566                     ; 166     clockfrequency = HSI_VALUE / presc;
 ...
 589                     ; 168     return((uint32_t)clockfrequency);
