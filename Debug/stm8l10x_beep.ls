  ...
  43                     ; 45 void BEEP_DeInit(void)
  43                     ; 46 {
  45                     	switch	.text
  46  0000               _BEEP_DeInit:
  50                     ; 47     BEEP->CSR = BEEP_CSR_RESET_VALUE;
  52  0000 351f50f3      	mov	20723,#31
  53                     ; 48 }
  56  0004 81            	ret
 121                     ; 58 void BEEP_Init(BEEP_Frequency_TypeDef BEEP_Frequency)
 121                     ; 59 {
 122                     	switch	.text
 123  0005               _BEEP_Init:
 125  0005 88            	push	a
 126       00000000      OFST:	set	0
 129                     ; 62     assert_param(IS_BEEP_FREQUENCY(BEEP_Frequency));
 131                     ; 65     if ((BEEP->CSR & BEEP_CSR_BEEPDIV) == BEEP_CSR_BEEPDIV)
 133  0006 c650f3        	ld	a,20723
 134  0009 a41f          	and	a,#31
 135  000b a11f          	cp	a,#31
 136  000d 260c          	jrne	L15
 137                     ; 67         BEEP->CSR &= (uint8_t)(~BEEP_CSR_BEEPDIV); /* Clear bits */
 139  000f c650f3        	ld	a,20723
 140  0012 a4e0          	and	a,#224
 141  0014 c750f3        	ld	20723,a
 142                     ; 68         BEEP->CSR |= BEEP_CALIBRATION_DEFAULT;
 144  0017 721050f3      	bset	20723,#0
 145  001b               L15:
 146                     ; 72     BEEP->CSR &= (uint8_t)(~BEEP_CSR_BEEPSEL);
 148  001b c650f3        	ld	a,20723
 149  001e a43f          	and	a,#63
 150  0020 c750f3        	ld	20723,a
 151                     ; 73     BEEP->CSR |= (uint8_t)(BEEP_Frequency);
 ...
 159  002b 84            	pop	a
 160  002c 81            	ret
 215                     ; 84 void BEEP_Cmd(FunctionalState NewState)
 215                     ; 85 {
 216                     	switch	.text
 217  002d               _BEEP_Cmd:
 221                     ; 86     assert_param(IS_FUNCTIONAL_STATE(NewState));
 223                     ; 88     if (NewState != DISABLE)
 225  002d 4d            	tnz	a
 226  002e 2706          	jreq	L101
 227                     ; 91         BEEP->CSR |= BEEP_CSR_BEEPEN;
 229  0030 721a50f3      	bset	20723,#5
 231  0034 2004          	jra	L301
 232  0036               L101:
 233                     ; 96         BEEP->CSR &= (uint8_t)(~BEEP_CSR_BEEPEN);
 ...
 296                     ; 113 void BEEP_LSICalibrationConfig(uint32_t LSIFreqHz)
 296                     ; 114 {
 297                     	switch	.text
 298  003b               _BEEP_LSICalibrationConfig:
 300  003b 5206          	subw	sp,#6
 301       00000006      OFST:	set	6
 304                     ; 120     assert_param(IS_LSI_FREQUENCY(LSIFreqHz));
 306                     ; 122     lsifreqkhz = (uint16_t)(LSIFreqHz / 1000); /* Converts value in kHz */
 ...
 317                     ; 126     BEEP->CSR &= (uint8_t)(~BEEP_CSR_BEEPDIV); /* Clear bits */
 319  004e c650f3        	ld	a,20723
 320  0051 a4e0          	and	a,#224
 321  0053 c750f3        	ld	20723,a
 322                     ; 128     A = (uint16_t)(lsifreqkhz >> 3U); /* Division by 8, keep integer part only */
 324  0056 1e03          	ldw	x,(OFST-3,sp)
 325  0058 54            	srlw	x
 326  0059 54            	srlw	x
 327  005a 54            	srlw	x
 328  005b 1f05          	ldw	(OFST-1,sp),x
 329                     ; 130     if ((8U * A) >= ((lsifreqkhz - (8U * A)) * (1U + (2U * A))))
 ...
 358  008d               L331:
 359                     ; 136         BEEP->CSR |= (uint8_t)(A - 1U);
 361  008d 7b06          	ld	a,(OFST+0,sp)
 362  008f 4a            	dec	a
 363  0090 ca50f3        	or	a,20723
 364  0093 c750f3        	ld	20723,a
 365  0096               L531:
 366                     ; 140     AWU->CSR |= AWU_CSR_MR;
 368  0096 721250f0      	bset	20720,#1
 369                     ; 142 }
